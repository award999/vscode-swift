...
function waitForResolveUpdated(ctx: WorkspaceContext) {
    return new Promise<void>(res => {
        const disposable = ctx.onDidChangeFolders(({ operation }) => {
            if (operation === FolderOperation.resolvedUpdated) {
                disposable?.dispose();
                res();
            }
        });
...
function waitForDidChangeTreeData(provider: ProjectPanelProvider) {
    return new Promise<void>(res => {
...
for (let i = 0; i < 25; ++i) {
    suite.only("Dependency Commmands Test Suite @slow " + i, function () {
        ...
        suite("Swift: Use Local Dependency", function () {
            ...
            async function getDependencyInState(state: "remote" | "editing") {
                for (let i = 0; i < 10; i++) {
                    await workspaceContext.focusFolder(depsContext);
                    await vscode.commands.executeCommand(Commands.SHOW_NESTED_DEPENDENCIES_LIST);
            ...
            async function useLocalDependencyTest() {
                ...
                const promise = Promise.all([
                    waitForResolveUpdated(workspaceContext),
                    waitForDidChangeTreeData(treeProvider),
                ]);
